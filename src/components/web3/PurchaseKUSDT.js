...
export const purchaseKusdt = async (tokenId, referer) => {
    ...
    async function purchase(tokenId, referer) {
        ...
        return await window.contract.methods.purchaseERC20(nftAddr, usdtAddr, tokenId, referer, metaverseFee.toString(), prevFee.toString(), refFee.toString()).send({ from: account }).on('transactionHash', function (hash) {
