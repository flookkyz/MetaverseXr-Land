...
export const purchaseBatchEth = async (tokenId, referer) => {
    ...
    async function purchase(tokenId, referer) {
        ...
        return await window.contract.methods.purchaseERC20Batch(nftAddr, ethAddr, tokenId, referer, avgMetaverseFee.toString(), prevFee, totalRefFee.toString()).send({ from: account }).on('transactionHash', function (hash) {
