...
import { marketAddr, marketABI, usdtAddr, nftAddr, givenProvider } from './Contract';

export const purchaseBatchUsdt = async (tokenId, referer) => {
    ...
    async function purchase(tokenId, referer) {
        ...
        return await window.contract.methods.purchaseERC20Batch(nftAddr, usdtAddr, tokenId, referer, avgMetaverseFee.toString(), prevFee, totalRefFee.toString()).send({ from: account }).on('transactionHash', function (hash) {
